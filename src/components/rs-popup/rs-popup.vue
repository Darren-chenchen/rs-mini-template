<template>
  <view :id="[popupId ? 'lupopupWrapper' + popupId : '']" class="rs-popup-wrapper" :class="popupClass" :style="[popupStyles]">
    <view class="rs-popup-content" :class="popupContentClass" :style="[{ height: height, width: width, backgroundColor: backgroundColor }]">
      <slot></slot>
    </view>
    <view v-if="maskShow" class="rs-popup-mask" :class="popupMaskClass" @tap="close(maskClick)"></view>
  </view>
</template>
  ...
  computed: {
    ...
    popupContentClass: function() {
      ...
      if (!!this.newTransition && arrTransition.indexOf(this.transition) !== -1 && this.transition !== 'none') {
        _class += 'rs-popup-transition-' + this.transition
      }
      return _class
    },
