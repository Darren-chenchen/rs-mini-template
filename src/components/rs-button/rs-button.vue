<template>
  <button
    ...
    :class="[
      ...
      plain ? 'rs-' + type + '-plain' : '',
      loading ? 'rs-loading' : '',
      shape == 'circle' ? 'rs-round-circle' : '',
      hairLine ? showHairLineBorder : 'rs-bold-border'
    ]"
    :disabled="disabled"
    :form-type="formType"
    :open-type="openType"
...
export default {
  ...
  computed: {
    ...
    getHoverClass() {
      ...
      let hoverClass = ''
      hoverClass = this.plain ? 'rs-' + this.type + '-plain-hover' : 'rs-' + this.type + '-hover'
      return hoverClass
    },
    // 按钮主题
    buttonStyle() {
  ...
  methods: {
    ...
    getElQuery() {
      return new Promise(resolve => {
        ...
        queryInfo.select('.rs-btn').boundingClientRect()
        queryInfo.exec(data => {
          resolve(data)
        })
      })
