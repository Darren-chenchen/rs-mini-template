...
MeScroll.prototype.extendDownScroll = function (optDown) {
  ...
  MeScroll.extend(optDown, {
    ...
    native: false, // 是否使用系统自带的下拉刷新 默认false 仅mescroll-body生效 (值为true时,还需在pages配置enablePullDownRefresh:true详请参考mescroll-native的案例)
    autoShowLoading: false, // 如果设置auto=true(在初始化完毕之后自动执行下拉刷新的回调),那么是否显示下拉刷新的进度 默认false
    isLock: false, // 是否锁定下拉刷新,默认false
    offset: 80, // 在列表顶部,下拉大于80px,松手即可触发下拉刷新的回调
    startTop: 100, // scroll-view滚动到顶部时,此时的scroll-top不一定为0, 此值用于控制最大的误差
    fps: 80, // 下拉节流 (值越大每秒刷新频率越高)
    inOffsetRate: 1, // 在列表顶部,下拉的距离小于offset时,改变下拉区域高度比例值小于1且越接近0,高度变化越小,表现为越往下越难拉
    outOffsetRate: 0.2, // 在列表顶部,下拉的距离大于offset时,改变下拉区域高度比例值小于1且越接近0,高度变化越小,表现为越往下越难拉
    bottomOffset: 20, // 当手指touchmove位置在距离body底部20px范围内的时候结束上拉刷新,避免Webview嵌套导致touchend事件不执行
    minAngle: 45, // 向下滑动最少偏移的角度,取值区间  [0,90]默认45度,即向下滑动的角度大于45度则触发下拉而小于45度,将不触发下拉,避免与左右滑动的轮播等组件冲突
...
MeScroll.prototype.extendUpScroll = function (optUp) {
  ...
  MeScroll.extend(optUp, {
    ...
    isBoth: true, // 上拉加载时,如果滑动到列表顶部是否可以同时触发下拉刷新默认true,两者可同时触发
    isBounce: false, // 默认禁止橡皮筋的回弹效果, 必读事项: http://www.mescroll.com/qa.html?v=190725#q25
    callback: null, // 上拉加载的回调function(page,mescroll){ }
    page: {
      num: 0, // 当前页码,默认0,回调之前会加1,即callback(page)会从1开始
      size: 10, // 每页数据的数量
      time: null // 加载第一页数据服务器返回的时间 防止用户翻页时,后台新增了数据从而导致下一页数据重复
    },
    noMoreSize: 5, // 如果列表已无数据,可设置列表的总数量要大于等于5条才显示无更多数据避免列表数据过少(比如只有一条数据),显示无更多数据会不好看
    offset: 0, // 距底部多远时,触发upCallback
    textLoading: '加载中 ...', // 加载中的提示文本
...
/* 锁定下拉刷新:isLock=ture,null锁定isLock=false解锁 */
MeScroll.prototype.lockDownScroll = function (isLock) {
  if (isLock == null) isLock = true
  this.optDown.isLock = isLock
}

/* 锁定上拉加载:isLock=ture,null锁定isLock=false解锁 */
 ...
 * hasNext: 是否还有下一页,布尔类型用来解决这个小问题:比如列表共有20条数据,每页加载10条,共2页.如果只根据dataSize判断,则需翻到第三页才会知道无更多数据,如果传了hasNext,则翻到第二页即可显示无更多数据.
 * systime: 服务器时间(可空)用来解决这个小问题:当准备翻下一页时,数据库新增了几条记录,此时翻下一页,前面的几条数据会和上一页的重复这里传入了systime,那么upCallback的page.time就会有值,把page.time传给服务器,让后台过滤新加入的那几条记录
 */
...
/* 计步器
 ...
 t: 计步时长,传0则直接回调end值不传则默认300ms
 rate: 周期不传则默认30ms计步一次
 * */
