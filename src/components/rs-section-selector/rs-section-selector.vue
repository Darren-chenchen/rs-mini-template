<template>
  <view class="rs-subsection" :style="[subsectionStyle]">
    <view
      class="rs-item rs-clamp-1"
      :style="[itemStyle(index)]"
      @tap="click(index)"
      :class="[noBorderRight(index), 'rs-item-' + index]"
      v-for="(item, index) in listInfo"
      :key="index"
    >
      <view :style="[textStyle(index)]" class="rs-item-text rs-clamp-1">{{ item.name }}</view>
    </view>
    <view class="rs-item-bg" :style="[itemBarStyle]"></view>
  </view>
</template>
  ...
  computed: {
    ...
    noBorderRight() {
      return index => {
        ...
        let classs = ''
        // 不显示右边的边框
        if (index < this.list.length - 1) classs += ' rs-none-border-right'
        // 显示整个组件的左右边圆角
        if (index === 0) classs += ' rs-item-first'
        if (index === this.list.length - 1) classs += ' rs-item-last'
        return classs
      }
    },
    // 文字的样式
  ...
  methods: {
    ...
    getTabsInfo() {
      ...
      for (let i = 0; i < this.list.length; i++) {
        view.select('.rs-item-' + i).boundingClientRect()
      }
      view.exec(res => {
        if (!res.length) {
          setTimeout(() => {
